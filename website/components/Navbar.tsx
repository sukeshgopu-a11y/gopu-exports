    ...
    <header className="sticky top-0 z-50 border-b border-[#D9E2EC] bg-white/95 shadow-sm backdrop-blur">
      ...
      <div className="mx-auto flex max-w-[1450px] items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:py-4">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          <Link href="/contact" className="rounded-xl bg-[#0E7490] px-5 py-3 text-[11px] font-black uppercase tracking-[0.12em] text-white shadow-sm transition hover:bg-[#0A5A70] hover:shadow-md">
            Request Quote
          </Link>
        </div>
      ...
      {menuOpen && (
        <div className="border-t border-[#E2E8F0] bg-white px-5 py-5 shadow-xl lg:hidden">
          <nav className="grid gap-1">
            {MAIN_LINKS.map(([label, href]) => (
              <Link key={href} href={href} onClick={() => setMenuOpen(false)} className="rounded-xl px-3 py-3 text-[12px] font-black uppercase tracking-[0.12em] text-slate-800 hover:bg-[#F0F9FA] hover:text-[#0E7490]">
                {label}
              </Link>
            ))}
          </nav>
