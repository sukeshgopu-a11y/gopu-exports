    ...
    <main className="bg-[#F5F7FA] text-[#0F172A]">
      ...
      <section className="border-y border-[#D9E2EC] bg-white">
        <div className="mx-auto grid max-w-[1280px] gap-8 px-6 py-12 sm:px-8 lg:grid-cols-2">
          <div><h2 className="text-3xl font-bold">Your export partner in India</h2><p className="mt-4 text-base leading-7 text-[#475569]">From product selection to shipment, work with one export team on specifications, packaging and order documents. Our Hyderabad head office is your contact for Indian agricultural exports.</p><Link href="/about" className="mt-4 inline-block font-bold text-[#0E7490]">About GOPU Exports →</Link></div>
          <div><p className="font-bold">{COMPANY.legalName}</p><dl className="mt-4 grid gap-2 text-sm sm:grid-cols-3">{[["IEC", COMPANY.iec], ["GSTIN", COMPANY.gst], ["CIN", COMPANY.cin]].map(([label, value]) => <div key={label} className="rounded-lg bg-[#F5F7FA] p-3"><dt className="font-bold text-[#0E7490]">{label}</dt><dd className="mt-1 break-all">{value}</dd></div>)}</dl><Link href="/company-verification" className="mt-4 inline-block font-bold text-[#0E7490]">Review company documents →</Link><p className="mt-2 text-sm text-[#475569]">Product testing and inspection requirements are agreed for each order.</p></div>
        </div>
      </section>

      <section className="mx-auto max-w-[1280px] px-6 py-12 sm:px-8">
        <h2 className="text-3xl font-bold">From your enquiry to shipment</h2>
      ...
      <section className="bg-white">
        <div className="mx-auto grid max-w-[1280px] gap-8 px-6 py-12 sm:px-8 lg:grid-cols-2">
          <div><h2 className="text-3xl font-bold">Planning your next import?</h2><p className="mt-4 text-base leading-7 text-[#475569]">Tell us your destination country and delivery port. We review product availability, packaging and shipment requirements before confirming your order.</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-base font-bold text-[#0E7490]"><Link href="/markets">International buyer information →</Link><Link href="/resources/packaging-standards">Packing options →</Link></div><p className="mt-6 text-base leading-7 text-[#475569]">Explore <Link className="underline" href="/export/agricultural-exporter-hyderabad-telangana">our Hyderabad export business</Link> and <Link className="underline" href="/export/andhra-pradesh-chilli-rice-exports">chilli and rice from Andhra Pradesh</Link>.</p></div>
          <div className="divide-y divide-[#D9E2EC]">{faqs.map(([question, answer]) => <details key={question} className="py-4 first:pt-0"><summary className="cursor-pointer text-base font-bold leading-7">{question}</summary><p className="mt-3 text-base leading-7 text-[#475569]">{answer}</p></details>)}</div>
        </div>
      </section>
